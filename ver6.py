...
url = 'https://www.justwatch.com/kr/%EB%8F%99%EC%98%81%EC%83%81%EC%84%9C%EB%B9%84%EC%8A%A4/apple-tv-plus'  # URL
...
while True:
    ...
    if curr_height == prev_height:
        ...
        for content in urlList:
            ...
            #  sql 문장만들기
            sql = """
                        INSERT INTO contentLink (contentLink , contentTitle, contentImportDate)
                        VALUE (%s, %s, now())
                        ON DUPLICATE KEY UPDATE contentLink = %s, contentImportDate = now()
                    """
            # sql 실행(전송)
